...
Convert the user's image request into one JSON object and output JSON only.
The only supported image model is illustrious.
Use exactly these fields:
{"model":"illustrious","positive_prompt":"...","negative_prompt":"...","status":"over"}
...
class OllamaClient:
    ...
    def generate_prompt(self, user_text: str) -> PromptPlan:
        payload = {"model": self.model, "stream": False, "format": "json", "messages": [
        ...
        return PromptPlan(result["model"].strip().lower(), result["positive_prompt"].strip(), result["negative_prompt"].strip(), result["status"].strip().lower())

    def _post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        request = Request(f"{self.base_url}{path}", data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), headers={"Content-Type": "application/json; charset=utf-8"}, method="POST")
        try:
